...
def get_total_year(branch, year):
    ...
    total_year = (
        ...
        sum((event.out_of_station or 0) for event in events_year) +
        sum((report.qty_visited_out_station or 0) for report in visit_reports_year)
    )
    return total_year
...
def generate_digital_month_report(user, month):
    ...
    ws['C19'] = sum((report.qty_visited_14 or 0) + (report.qty_visited_15_35 or 0) + (report.qty_visited_other or 0) -
                    (report.qty_visited_out_station or 0) for report in visit_reports)
