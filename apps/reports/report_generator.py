...
def generate_book_report_excel(user, year, month):
    ...
    if len(daily_data) > 1:
        for day, data in sorted(daily_data.items())[1:]:
            ...
            ws[f'L{row_num}'] = data['qty_books_part_opl'] + data['qty_books_part_enm'] + data[
                'qty_books_part_tech']
            + data['qty_books_part_sh'] + data['qty_books_part_si'] + data['qty_books_part_yl'] + \
            data['qty_books_part_hl']
            + data['qty_books_part_dl'] + data['qty_books_part_other'] + data[
                'qty_books_part_audio'] + data['qty_books_part_krai']
