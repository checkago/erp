...
from .utils import get_totals, get_book_totals, get_event_totals, get_notes_with_data
...
class DiaryView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['notes'] = get_notes_with_data()
        return context
...
class VisitReportListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        user = self.request.user
        employee = Employee.objects.get(user=user)
        return VisitReport.objects.filter(library=employee.branch).order_by('-date')[:15]
...
class BookReportListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return BookReport.objects.filter(library=employee.branch).order_by('-date')[:15]
