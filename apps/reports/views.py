...
class EventListView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        date_160_days_ago = timezone.now() - timedelta(days=160)
        events = Event.objects.filter(date__gte=date_160_days_ago, library=employee.branch).order_by('-date')
...
class VisitReportListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        employee = Employee.objects.get(user=user)
        date_160_days_ago = timezone.now() - timedelta(days=160)
        return VisitReport.objects.filter(date__gte=date_160_days_ago, library=employee.branch).order_by('-date')
...
class BookReportListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        employee = Employee.objects.get(user=user)
        date_160_days_ago = timezone.now() - timedelta(days=160)
        return BookReport.objects.filter(date__gte=date_160_days_ago, library=employee.branch).order_by('-date')
